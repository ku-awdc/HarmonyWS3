...
#' subtitle: Homework and software installation for the HARMONY training workshop on diagnostic test evaluation
#' fontsize: 12pt
#' ---
#' 
...
#' 
#' You should see a number of folders listed with names "Session_0", "Session_1" etc, and within these folders are files like "Session_1.Rmd", "Session_1.pdf" etc.  Each of the sessions for the training workshop (including this preparation document) has a series of files:  the .Rmd is the original Rmarkdown document, and the .pdf & .html & .R files are created from these.  You can click on any of these files to view them, although of the different file formats the .pdf version is probably easiest to download directly from the website.  There are two problems with this:
#' 
...
#' For example, look inside the `'Session_0'` folder and open up the `'Session_Preparation.html'` file - that is a local copy of the same document you are currently reading! But now you also have the PDF version - use this if you want to print the document for some reason, but it is a good idea to stick to using the HTML version if you want to copy/paste R code (you will probably encounter problems with quotation marks if you copy/paste R code from PDF files).  Alternatively, you can use the .R files to run the R code directly.  The code in each of the three versions should be identical.
#' 
...
#' 
## ---- fig.width=6, fig.height=8, echo=FALSE-----------------------------------
par(mfrow=c(2,1))
with(results, plot(parameter, posterior, type='l', 
      xlim=c(0,1), main='True posterior', ylab=NA, xlab=NA))
plot(density(samples), xlim=c(0,1), main='Sampled posterior', 
...
#' 
## ----eval=FALSE, include=FALSE, show=FALSE------------------------------------
