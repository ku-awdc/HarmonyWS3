...
You need to install R (version 4.1.0 or later; preferably version 4.2.0) from https://cran.r-project.org/ and we recommend that you also use Rstudio which can be downloaded separately from https://www.rstudio.com/products/rstudio/download/
...
TODO:  Use RStudio for git cloning - Arianna [DONE]
TODO:  add link to happy with GIT
TODO: mention that they need to download git (and configure it) in order to use it with RStudio (unless using a MAC or a Linux distro that comes with git installed by default).
...
You should see a number of folders listed with names "Day0", "Day1" etc, and within these folders are files like like "Session_1.Rmd", "Session_1.pdf" etc.  Each of the sessions for the training workshop (including this preparation document) has a series of files:  the .Rmd is the original Rmarkdown document, and the .pdf and .html files are created from these.  You can click on any of these files to view them, although of the different file formats the .pdf version is probably easiest to download directly from the website.  There are two problems with this:
...
For example, look inside the `'Day0'` folder and open up the `'Session_Preparation.html'` file - that is a local copy of the same document you are currently reading! But now you also have the PDF version - use this if you want to print the document for some reason, but it is a good idea to stick to using the HTML version if you want to copy/paste R code (you will probably encounter problems with quotation marks if you copy/paste R code from PDF files).  The two versions should be identical.  
